...
export default class WorkList {
	...
	renderList() {
		this.listData.forEach(_work => {//便利的就是外部传入的数据
			const _html_ = this.createWorkDOM(_work) //🔥🔥渲染好数据的 DOM, 然后再传回到 append 方法中去挂载回 DOM 树上！
			this.append(_html_)
		})
	}
	...
	static init(data) { //🔥🔥！！data 是外部传入的数据
		if(this.createList instanceof  WorkList) {//⚡️⚡️单实例的模式, 说明已经创建过实例了, 避免重复创建
			...
			this.createListDOM = new WorkList(data) //🔥🔥创建一个实例！！！  静态方法、静态属性的 this 指向类本身, 而不是实例, 原型方法的 this 访问的是实例！！!
			this.createListDOM.renderList()
		}
	}
